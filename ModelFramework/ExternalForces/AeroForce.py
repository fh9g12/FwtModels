...
class AeroForce(ExternalForce):
    ...
    def Generate(cls,FwtParams,Transform,C_L,int_tuple,alphadot,M_thetadot,e,rootAlpha,deltaAlpha,alpha_zero = 0):
        ...
        BodyJacobian = Transform.InvAdjoint()*Transform.ManipJacobian(p.q)
        ...
        Q_L = (velocity_frame.ManipJacobian(p.q)).T
        Q_L *= velocity_frame.InvAdjoint().T
        Q_L =cls._trigsimp(Q_L)
        Q_L *= wrench_lift
        ...
        Q_M = (velocity_frame.ManipJacobian(p.q)).T
        Q_M *= velocity_frame.InvAdjoint().T
        Q_M = cls._trigsimp(Q_M)
        Q_M *= wrench_moment
